...
export const uploadAadhar =
  ({ aadhar_front_image, aadhar_back_image, name, aadhar_no }) =>
  async (dispatch) => {
    // try {
...
export const uploadPan =
  ({ pan_front, pan_back, name, pan_no }) =>
  async (dispatch) => {
    // try {
    dispatch({ type: DOCUMENT_LOADING });
...
export const uploadRc =
  ({ rc_front, rc_back, name, mfd_date, rc_no }) =>
  async (dispatch) => {
    // try {
    dispatch({ type: DOCUMENT_LOADING });
    ...
    data.append("rc_front", {
      name: rc_front,
      uri:
        Platform.OS === "android" ? rc_front : rc_front.replace("file://", ""),
      type: "image/jpeg",
    });
    data.append("rc_back", {
      name: rc_back,
...
export const uploadAccountDetail =
  ({ bank_name, account_type, bank_IFSC, account_no, mobile_no, name, id }) =>
  async (dispatch) => {
    try {
      dispatch({ type: DOCUMENT_LOADING });
...
export const TaxiPhoto =
  ...
  async (dispatch) => {
    try {
      ...
      data.append("backtend_image", {
        name: Texiback,
        uri:
          Platform.OS === "android"
            ? Texiback
            : Texiback.replace("file://", ""),
        type: "image/jpeg",
      });
